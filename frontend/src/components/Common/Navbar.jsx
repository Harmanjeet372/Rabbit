...
const Navbar = () => {
  ...
  return (
    <>
      <nav className='container mx-auto flex items-center justify-between py-4 px-6'>
        ...
        <div className='flex items-center space-x-4'>
          ...
          <button onClick={toggleCartDrawer} className='relative hover:text-black' aria-label="Open Cart">
            ...
            <span className='absolute -top-1 -right-2 bg-rabbit-red text-white text-[10px] font-semibold rounded-full w-5 h-5 flex items-center justify-center'>
              4
            </span>
          </button>
      ...
      {/* Mobile Navigation */}
      <div className={`fixed top-0 left-0 w-3/4 sm:w-1/3 h-full bg-white/95 backdrop-blur shadow-lg transform transition-transform duration-300 z-40 ${navDrawerOpen ? "translate-x-0" : "-translate-x-full"}`}>
